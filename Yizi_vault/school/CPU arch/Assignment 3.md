...
#### Table As Requested In The Assignment!
...
1. **Pipeline stalling :**
	We stop the flow of the pipeline for instructions that were added later by  that are in earlier stages of the pipeline (by inserting `NOP`s...) while dealing with the instruction that causes the hazard. Once that instruction is finished we stop the stalling and work as usual.
	- **Pros :** It is a very easy instruction to implement that can prevent all types of hazards.
	- **Cons :** This solution hurt performance directly. Stalling the program necessarily increases runtime.

2. **Forwarding :** 
	This method handles **Data Hazards** in an elegant way. We give early access to computed values to instructions that are in different stages of the pipeline instead of making them wait the entire time its will take to writeback the computed values.
	- **Pros :** Drastically reduces if not eliminates data stalls.
	- **Cons :** Additional hardware costs for forwarding data between different stages of the pipeline.

3. **Instruction Scheduling (Compiler Reordering) :**
	Instead of delegating the responsibility of optimizing to external hardware we use software to find delay slots (precalculated delays between dependent instructions) and insert other instructions that are independent of any hazardous adjacent instruction. This allows the CPU to take advantage of time slots that are not originally utilized.
	- **Pros :** Improves performance without any hardware costs.
	- **Cons :** This method does not resolve all cases. For example, it does not resolve structural hazard all the time since it is not necessarily possible to know if 2 memory addresses will collide at runtime.

4. **Branching Prediction :**
	We add additional hardware that helps us optimize the handling of Control Hazards by predicting results of branching instructions (Examples discussed in previous parts of the assignment...). 
	- **Pros :** Enhances performance by reducing Control stalls.
	- **Cons :** Additional hardware costs.
